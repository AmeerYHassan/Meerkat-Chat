"""the server file for the chat app"""
# pylint: disable=E1101, C0325, C0103
# pylint E1101 and C0325 disabled as they were false positives
# pylint C0103 is for snake case, but would cause database issues
# if changed now
...
def getBotResponse(returnObject, message):
    """method that creates a response for the bot"""
    splitMessage = message.split()
    # If the first part of the message has funtranslate, take the rest of the
    # message and send an API call to the funtranslate API
    if "funtranslate" in splitMessage[0]:
        ...
        if translateString.replace(' ', '').isalpha():
            ...
            if (response.status_code == 200):
                returnObject["message"] = response.json()[
                    "contents"]["translated"]
            else:
                returnObject["message"] = \
                    "Uh oh! Try again in a few, I can't seem to translate that right now."
        else:
            returnObject["message"] = \
                "Sorry, I only can translate sentences that just contain letters! Try again"
    # If the first part specifies catfact, get a random cat fact from the API
    # and return to the server
        ...
        if (response.status_code == 200):
            ...
            returnObject["message"] = \
                "Uh oh! Try again in a few, I can't seem to get a cat fact right now."
    # Return a gif if giphy is in the command.
    elif "giphy" in splitMessage[0]:
        query = "%20".join(splitMessage[1:])
        requestUrl = "https://api.giphy.com/v1/gifs/search?api_key=" + \
            giphy_key + "&q=" + query + "&limit=10&offset=0&rating=g&lang=en"
        response = requests.get(requestUrl)
        if (response.status_code == 200):
            returnObject["hasImage"] = True
            returnObject["imageLink"] = random.choice(
                response.json()["data"])["images"]["original"]["url"]
        else:
            returnObject["message"] = \
                "Uh oh! Try again in a few, I can't seem to get a gif right now."
    # Return the current time
    elif "time" in splitMessage[0]:
        returnObject["message"] = "The current time is " + \
            datetime.now().strftime("%H:%M:%S")
    # Return the about section of the bot
    elif "about" in splitMessage[0]:
        returnObject["message"] = \
            "Hi, I'm meerkat, and I'm just a small utility for this chat room!"\
...
def emit_all_messages():
    """emits all the messages to the users"""
    # Read all of the entries from the database, put each column in a list
    all_messages = [
        db_message.message for db_message in db.session.query(
            models.Messages).all()]
    all_users = [
        db_username.username for db_username in db.session.query(
            models.Messages).all()]
    all_bot_status = [
        db_isBot.isBot for db_isBot in db.session.query(
            models.Messages).all()]
    all_profile_pictures = [
        db_profilePicture.profilePicture for db_profilePicture in db.session.query(
            models.Messages).all()]
    all_hasImage = [
        db_hasImage.hasImage for db_hasImage in db.session.query(
            models.Messages).all()]
    all_hasLink = [
        db_hasLink.hasLink for db_hasLink in db.session.query(
            models.Messages).all()]
    all_imageLink = [
        db_imageLink.imageLink for db_imageLink in db.session.query(
            models.Messages).all()]
    all_hyperlink = [
        db_hyperlink.hyperlink for db_hyperlink in db.session.query(
            models.Messages).all()]

    messageObjects = []
    # Make a list of message objects, iterating through every list using an
    # index.
...
def emit_current_users():
    """emits all of the users to update connected users"""
    userObjects = []
...
# On connect, tell the client to increase the user count by one and emit
# all the messages.
@socketio.on('connect')
def on_connect():
    """emits all messages and users"""
    emit_all_messages()
    emit_current_users()
...
def on_disconnect():
    """removes user from list, emits the new user count"""
...
def new_message(data):
    """creates a new message object, writes to db on message recieved"""
    # Parse the message to see if it's a bot command or a user message. Create
    # an object to hold message contents.
    messageContents = data["message"]
    retObj = {}
    # If it's a bot, call the bot response method and set the object to
    # specify that it's a bot.
    if (messageContents[0:2] == "!!"):
        retObj = {
            ...
            "profilePicture": \
                ...
                "cliparts/68083/meerkat-face-clipart-xl.png",
            "hasImage": False,
            "hasLink": False,
            "hyperlink": None,
            "imageLink": None
    ...
    # If it's a user, either call their username from the dictionary above or
    # create a username for them.
        ...
        for word in splitMessage:
            if "https://" in word or "http://" in word:
                if word[-3:] == "jpg" or word[-3:] == "png" \
                    or word[-3:] == "gif" or word[-4:] == "jpeg":
                    hasImage = True
                    imageLink = word
                else:
                    hasLink = True
        ...
        retObj["isBot"] = False
        retObj["username"] = \
            usernameDict[flask.request.sid]["username"]
        retObj["profilePicture"] = \
            usernameDict[flask.request.sid]["profilePicture"]
        retObj["hasImage"] = hasImage
        retObj["hasLink"] = hasLink
        retObj["hyperlink"] = hyperlink
        retObj["imageLink"] = imageLink

    # Write to the database the contents of the message, the username, and
    # whether or not it's a bot message.
...
def user_login(data):
    """method that unlocks the channel, announces login"""
    retObj = {}
...
@app.route('/')
def index():
    """return the html of the generated html"""
